...
namespace GerberLibrary
{
    ...
    public class GerberParserState
    {
        ...
        public GerberLibrary.Core.InterpolationMode MoveInterpolation = GerberLibrary.Core.InterpolationMode.Linear;
    ...
    public partial class PolyLineSet
    {
        ...
        public static ParsedGerber ParseGerber274x(ProgressLog log, List<String> inputlines, bool parseonly, bool forcezerowidth = false, GerberParserState State = null)
        {
            ...
            if (State.PreCombinePolygons)
            {
                ...
                for (int i = 0; i < solution.Count; i++)
                {
                    ...
                    PL.MyColor = Color.FromRgba(200, 128, 0, 255);
        ...
        private static bool BasicLineCommands(string Line, GerberParserState State)
        {
            ...
            switch (FinalLine)
            {
                ...
                case "G01":
                    State.MoveInterpolation = GerberLibrary.Core.InterpolationMode.Linear;
                    break;
                case "G02":
                ...
                if (BasicLineCommands(Line, State) == false)
                {
                    ...
                    if (GCC.charcommands.Count > 0 && GCC.errors == 0)
                    {
                        switch (GCC.charcommands[0])
                        {
                            ...
                            default:
                                // Check for FS format specification (needed for KiCad v8+)
                                if (GSHasChar(GCC, 'F') && GSHasChar(GCC, 'S') &&
                                    (GSHasChar(GCC, 'X') || GSHasChar(GCC, 'Y')))
                                {
                                    int xIdx = -1, yIdx = -1;
                                    for (int i = 0; i < GCC.charcommands.Count; i++)
                                    {
                                        if (GCC.charcommands[i] == 'X') xIdx = i;
                                        if (GCC.charcommands[i] == 'Y') yIdx = i;
                                    }
                                ...
                                // Check for X2 attributes (TF, TA, TO, TD) - silently skip
                                if (GCC.charcommands.Count >= 2 && GCC.charcommands[0] == 'T')
                                {
                                    if (GCC.charcommands[1] == 'F' || GCC.charcommands[1] == 'A' ||
                                        GCC.charcommands[1] == 'O' || GCC.charcommands[1] == 'D')
                                    {
                                        break;
                                    }
                                }

                                // Check for %IP, %OF, %LN - silently skip
                                if (GCC.charcommands.Count >= 2 && GCC.charcommands[0] == 'I' && GCC.charcommands[1] == 'P') break;
                                if (GCC.charcommands.Count >= 2 && GCC.charcommands[0] == 'O' && GCC.charcommands[1] == 'F') break;
                                if (GCC.charcommands.Count >= 2 && GCC.charcommands[0] == 'L' && GCC.charcommands[1] == 'N') break;
                                ...
                                else
                                {
                                    ...
                                    if (GS.Has("X") || GS.Has("Y") || GS.Has("D"))
                                    {
                                        ...
                                        if (State.MoveInterpolation != GerberLibrary.Core.InterpolationMode.Linear)
                                        {
                                        ...
                                        if (State.PolygonMode)
                                        {
                                            ...
                                            switch (ActualD)
                                            {
                                                case 1:
                                                    ...
                                                    {
                                                        case GerberLibrary.Core.InterpolationMode.Linear:
        ...
        private static bool GSHasChar(GCodeCommand gcc, char c)
        {
            for (int i = 0; i < gcc.charcommands.Count; i++)
                if (gcc.charcommands[i] == c) return true;
            return false;
        }
