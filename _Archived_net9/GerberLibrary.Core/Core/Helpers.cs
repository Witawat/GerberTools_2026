...
namespace GerberLibrary.Core
{
    public class Line
    {
        ...
        public void Draw(GraphicsInterface g, Color C, float W = 1.0f)
        {
    ...
    public static class Helpers
    {
        ...
        /*
        [Obsolete("This method uses Windows Forms PictureBox which is not cross-platform. Use alternative rendering approach.")]
        public static float SetupMatrixForExtends(Graphics G, object P, Bounds Bb, int pixelmargin = 0, float percentmargin = 0)
        {
            // This method is Windows Forms specific and not used in cross-platform scenarios
            throw new NotImplementedException("SetupMatrixForExtends requires Windows Forms PictureBox which is not cross-platform");
        }
        */
        ...
        public static void ColorToHSV(Color color, out double hue, out double saturation, out double value)
        {
            ...
            // hue = color.GetHue(); // System.Drawing specific
            // Manual Hue calculation:
            float r = c.R;
            float g = c.G;
            float b = c.B;
            float delta = max - min;
        ...
        public static Color ColorFromHSV(double hue, double saturation, double value)
        {
            ...
            if (hi == 0)
                return Color.FromRgba((byte)v, (byte)t, (byte)p, 255);
            else if (hi == 1)
                return Color.FromRgba((byte)q, (byte)v, (byte)p, 255);
            else if (hi == 2)
                return Color.FromRgba((byte)p, (byte)v, (byte)t, 255);
            else if (hi == 3)
                return Color.FromRgba((byte)p, (byte)q, (byte)v, 255);
            else if (hi == 4)
                return Color.FromRgba((byte)t, (byte)p, (byte)v, 255);
            else
                return Color.FromRgba((byte)v, (byte)p, (byte)q, 255);
        }
        ...
        public static Color Refraction(float Space)
        {
            ...
            return Color.FromRgba(RR, GG, BB, 255);
