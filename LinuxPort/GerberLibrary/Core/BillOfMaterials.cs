...
/*                var Pts = Root.Find("Board", false).First().Find("Points", false);
        ...
        */
        ...
        public void LoadKicad(string kicadschname, string kicadpcbname, StandardConsoleLog Log)
        {
                ...
                {
                    Log.AddString("Warning: No kicad_pcb root found in PCB file");
                    Log.PopActivity();
                    return;
                }
                ...
                foreach (var a in C)
                {
                    BoardSide side = BoardSide.Top;
                    var AT = a.Find("at");
                    if (AT == null || AT.Count == 0 || AT[0].Children.Count < 2) continue;
                ...
                for (int i =0;i<a.Children.Count();i++)
                {
                    ...
                    float Angle = 0;
                    float X = float.Parse(AT[0].Children[0].Name, CultureInfo.InvariantCulture);
                    float Y = float.Parse(AT[0].Children[1].Name, CultureInfo.InvariantCulture);
                    if (AT[0].Children.Count > 2)
                    {
                        Angle = float.Parse(AT[0].Children[2].Name, CultureInfo.InvariantCulture);
                    }
                    string reference = "";
                    string value = "";
