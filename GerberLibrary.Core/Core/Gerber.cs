...
namespace GerberLibrary
{
    ...
    public class BoardRenderColorSet
    {
        ...
        public Color BoardRenderCopperColor = Color.FromRgb(219, 125, 104);
        public Color BoardRenderPadColor = Gerber.ParseColor("gold");
        public Color BoardRenderSilkColor = Gerber.ParseColor("white");

        public Color BackgroundColor = Color.FromRgb(10, 10, 40);
        ...
        public Color GetDefaultColor(BoardLayer layer, BoardSide side)
        {
            switch (layer)
            {
                ...
                case BoardLayer.SolderMask: 
                    // Interpolate not directly available on Color, implementing simple substitute ? 
                    // For now, let's just pick one or do manual mix.
                    // ImageSharp Color is a struct, r,g,b are bytes.
                    return MixColors(BoardRenderColor, BoardRenderBaseMaterialColor, 0.2f);
                    
                case BoardLayer.Silk: return BoardRenderSilkColor;
            }
            return Color.FromRgba(255, 255, 255, 100);
        }

        private Color MixColors(Color c1, Color c2, float amount) 
        {
            // Simple linear interpolation
             Rgba32 r1 = c1.ToPixel<Rgba32>();
             Rgba32 r2 = c2.ToPixel<Rgba32>();
             byte r = (byte)(r1.R * (1-amount) + r2.R * amount);
             byte g = (byte)(r1.G * (1-amount) + r2.G * amount);
             byte b = (byte)(r1.B * (1-amount) + r2.B * amount);
             byte a = (byte)(r1.A * (1-amount) + r2.A * amount);
    ...
    public static class Gerber
    {
            ...
            if (Directory.Exists(BoardGerbersFolder))
            {
                ...
                if (OutlineMerge.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        FilesToZip.Add(TargetGerb);
                    }
                }
                
                // Create ZIP file using SharpZipLib
        ...
        public static Color ParseColor(string color)
        {
            ...
            {
                case "blue": return Color.FromRgb(0, 40, 74);
                case "yellow": return Color.FromRgb(234, 206, 39);
                case "green": return Color.FromRgb(0, 0x30, 0);
                case "black": return Color.FromRgb(5, 5, 5);
                case "white": return Color.FromRgb(250, 250, 250);
                case "red": return Color.FromRgb(192, 43, 43);
                case "silver": return Color.FromRgb(160, 160, 160);
                case "gold": return Color.FromRgb(239, 205, 85);
            }
            ...
            {
                // unknown colors end up here... 
            }

            return Color.Lime; // Default fall back
        }
        ...
        public static bool SaveDebugImage(string GerberFilename, string BitmapFilename, float dpi, Color Foreground, Color Background, ProgressLog log)
        {
            ...
            Image<Rgba32> B2 = GIC.RenderToImage(Width, Height, Tr, Foreground, Background, PLS, true);
            ...
            var GI = new ImageSharpGraphicsInterface(B2);
            GI.Transform = Tr;
            ...
            foreach (var L in GerberLines)
            {
                if (L[0] != '%')
                {
                    ...
                    if (GS.Has("G") && (int)GS.Get("G") == 3)
                    {
                        ...
                        DrawCross(GI, X, Y, Color.Blue);
                        DrawCross(GI, LastX, LastY, Color.Red);

                        DrawCross(GI, LastX + I, LastY - J, Color.Yellow);
                        DrawCross(GI, LastX + I, LastY + J, Color.Purple);
                        DrawCross(GI, LastX - I, LastY - J, Color.Green);
                        DrawCross(GI, LastX - I, LastY + J, Color.Orange);
        ...
        private static void DrawCross(GraphicsInterface g, double X, double Y, Color C)
        {
            float S = 0.2f;
            var P = new GerberLibrary.Core.Primitives.Pen(C, 1.0f);
            g.DrawLine(P, (float)X - S, (float)Y - S, (float)X + S, (float)Y - S);
            g.DrawLine(P, (float)X - S, (float)Y + S, (float)X + S, (float)Y + S);
            g.DrawLine(P, (float)X - S, (float)Y - S, (float)X - S, (float)Y + S);
            g.DrawLine(P, (float)X + S, (float)Y - S, (float)X + S, (float)Y + S);
        }
    ...
    public class LayerSet
    {
        ...
        public static List<LayerSet> LoadDefaultLayersetFromZip(string gerberFile)
        {
            ...
            using (ICSharpCode.SharpZipLib.Zip.ZipFile zip1 = new ICSharpCode.SharpZipLib.Zip.ZipFile(gerberFile))
            {
