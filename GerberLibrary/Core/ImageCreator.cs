      ...
      public  List<ParsedGerber> PLSs = new List<ParsedGerber>();
        ...
        public ParsedGerber AddBoardToSet(MemoryStream MS, string _originalfilename, ProgressLog log, bool forcezerowidth = false, bool precombinepolygons = false, double drillscaler = 1.0)
        {
            ...
            try
            {
                ...
                {
                    log.AddString(String.Format("Skipping {0}: unsupported file type", Path.GetFileName(_originalfilename)));
                    log.PopActivity();
                    return null;
                }
                ...
                PLS.CalcPathBounds();
                var bw = PLS.BoundingBox.Width();
                var bh = PLS.BoundingBox.Height();
                const double maxLayerSize = 10000.0;
                if (bw > maxLayerSize || bh > maxLayerSize || bw <= 0 || bh <= 0)
                {
                    log.AddString(String.Format("Warning: Skipping {0} - unreasonable bounding box: {1:N1} x {2:N1} mm (fmt={3}.{4} {5} OL={6})",
                        Path.GetFileName(_originalfilename), bw, bh,
                        State.CoordinateFormat.DigitsBefore, State.CoordinateFormat.DigitsAfter,
                        State.CoordinateFormat.CurrentNumberScale, State.CoordinateFormat.OmitLeading));
                    log.PopActivity();
                    return null;
                }
                BoundingBox.AddBox(PLS.BoundingBox);

                log.AddString(String.Format ("Loaded {0}: {1:N1} x {2:N1} mm (fmt={3}.{4} {5} OL={6})",
                    Path.GetFileName(_originalfilename),
            ...
            {
                log.AddString(String.Format("Exception loading {0}: {1}", Path.GetFileName(_originalfilename), E.Message));
                while (E != null)
                {
        ...
        public void DrawAllFiles(string v1, double dpi, ProgressLog Logger = null)
        {
            ...
            const double maxBoardMM = 5000.0;
            if (bw > maxBoardMM || bh > maxBoardMM)
            {
                Errors.Add(String.Format("Board size ({0:N0}x{1:N0} mm) exceeds maximum ({2:N0} mm).", bw, bh, maxBoardMM));
                if (Logger != null) Logger.AddString(String.Format("ERROR: Board size ({0:N0}x{1:N0} mm) exceeds maximum.", bw, bh));
                return;
            }
        ...
        Bitmap DrawBoard(double dpi, BoardSide CurrentLayer, BoardRenderColorSet Colors, string basefilename = null, ProgressLog Logger = null, bool ForceWhite = false)
        {
            ...
            try
            {
                ...
                if (bw > maxBoardMM || bh > maxBoardMM)
                {
                    Errors.Add(String.Format("Board size ({0:N0}x{1:N0} mm) exceeds maximum ({2:N0} mm). Possible corrupt Gerber coordinates.", bw, bh, maxBoardMM));
                    if (Logger != null) Logger.AddString(String.Format("ERROR: Board size ({0:N0}x{1:N0} mm) exceeds maximum.", bw, bh));
                    Logger.PopActivity();
                    return null;
                }
