...
namespace GerberLibrary
{
    ...
    public static class Gerber
    {
            ...
            switch (ext)
            {
                ...
                case "gbr":
                    ...
                    switch (Path.GetFileNameWithoutExtension(gerberfile).ToLower())
                    {
                        ...
                        // KiCad v8+ default naming (F_Cu.gbr, Edge_Cuts.gbr, etc.)
                        case "f_cu":
                        ...
                        case "f_courtyard":
                            Side = BoardSide.Top; Layer = BoardLayer.Courtyard;
                            break;
                        case "b_courtyard":
                            Side = BoardSide.Bottom; Layer = BoardLayer.Courtyard;
                            break;
                        case "f_fab":
                            Side = BoardSide.Top; Layer = BoardLayer.Fab;
                            break;
                        case "b_fab":
                            Side = BoardSide.Bottom; Layer = BoardLayer.Fab;
                            break;
                        case "user_drawings":
                            Side = BoardSide.Both; Layer = BoardLayer.UserDrawings;
                            break;
                        case "user_comments":
                            Side = BoardSide.Both; Layer = BoardLayer.UserComments;
                            break;
                        case "user_eco1":
                            Side = BoardSide.Both; Layer = BoardLayer.UserEco;
                            break;
                        case "user_eco2":
                            Side = BoardSide.Both; Layer = BoardLayer.UserEco;
                            break;
                        case "f_adhes":
                            Side = BoardSide.Top; Layer = BoardLayer.Utility;
                            break;
                        case "b_adhes":
                            Side = BoardSide.Bottom; Layer = BoardLayer.Utility;
                            break;

                        default:
                            {
                                string name = Path.GetFileNameWithoutExtension(gerberfile).ToLower();
                                ...
                                if (name.EndsWith("_edge_cuts") || lcase.Contains("-edge_cuts") || name == "edge_cuts") { Side = BoardSide.Both; Layer = BoardLayer.Outline; }
                                if (name.EndsWith("_b_cu") || lcase.Contains("-b_cu") || name == "b_cu") { Side = BoardSide.Bottom; Layer = BoardLayer.Copper; }
                                if (name.EndsWith("_f_cu") || lcase.Contains("-f_cu") || name == "f_cu") { Side = BoardSide.Top; Layer = BoardLayer.Copper; }
                                if (name.EndsWith("_b_silks") || name.EndsWith("_b_silkscreen") || lcase.Contains("-b_silks") || name == "b_silks" || name == "b_silkscreen") { Side = BoardSide.Bottom; Layer = BoardLayer.Silk; }
                                if (name.EndsWith("_f_silks") || name.EndsWith("_f_silkscreen") || lcase.Contains("-f_silks") || name == "f_silks" || name == "f_silkscreen") { Side = BoardSide.Top; Layer = BoardLayer.Silk; }
                                if (name.EndsWith("_b_mask") || lcase.Contains("-b_mask") || name == "b_mask") { Side = BoardSide.Bottom; Layer = BoardLayer.SolderMask; }
                                if (name.EndsWith("_f_mask") || lcase.Contains("-f_mask") || name == "f_mask") { Side = BoardSide.Top; Layer = BoardLayer.SolderMask; }
                                if (name.EndsWith("_b_paste") || lcase.Contains("-b_paste") || name == "b_paste") { Side = BoardSide.Bottom; Layer = BoardLayer.Paste; }
                                if (name.EndsWith("_f_paste") || lcase.Contains("-f_paste") || name == "f_paste") { Side = BoardSide.Top; Layer = BoardLayer.Paste; }
                                // KiCad v8+ internal layers (In1_Cu, In2_Cu, ...)
                                if (name.StartsWith("in") && name.EndsWith("_cu"))
                                {
                                    string numStr = name.Substring(2, name.Length - 6);
                                    if (int.TryParse(numStr, out int layerNum) && layerNum >= 1 && layerNum <= 20)
                                    {
                                        Side = (BoardSide)((int)BoardSide.Internal1 + layerNum - 1);
                                        Layer = BoardLayer.Copper;
                                    }
                                }
                                // KiCad v8+ courtyard layers
                                if (name.EndsWith("_f_courtyard") || lcase.Contains("-f_courtyard") || name == "f_courtyard") { Side = BoardSide.Top; Layer = BoardLayer.Courtyard; }
                                if (name.EndsWith("_b_courtyard") || lcase.Contains("-b_courtyard") || name == "b_courtyard") { Side = BoardSide.Bottom; Layer = BoardLayer.Courtyard; }
                                // KiCad v8+ fabrication layers
                                if (name.EndsWith("_f_fab") || lcase.Contains("-f_fab") || name == "f_fab") { Side = BoardSide.Top; Layer = BoardLayer.Fab; }
                                if (name.EndsWith("_b_fab") || lcase.Contains("-b_fab") || name == "b_fab") { Side = BoardSide.Bottom; Layer = BoardLayer.Fab; }
                                // KiCad v8+ adhesive layers
                                if (name.EndsWith("_f_adhes") || lcase.Contains("-f_adhes") || name == "f_adhes") { Side = BoardSide.Top; Layer = BoardLayer.Utility; }
                                if (name.EndsWith("_b_adhes") || lcase.Contains("-b_adhes") || name == "b_adhes") { Side = BoardSide.Bottom; Layer = BoardLayer.Utility; }
                                // KiCad v8+ user layers (User_1 to User_9)
                                if (name.StartsWith("user_") && name.Length > 5)
                                {
                                    string userNum = name.Substring(5);
                                    if (int.TryParse(userNum, out int uNum) && uNum >= 1 && uNum <= 9)
                                    {
                                        Side = BoardSide.Both; Layer = BoardLayer.UserDrawings;
                                    }
                                }
                                // KiCad v8+ user ECO layers
                                if (name.EndsWith("_user_eco1") || name == "user_eco1") { Side = BoardSide.Both; Layer = BoardLayer.UserEco; }
                                if (name.EndsWith("_user_eco2") || name == "user_eco2") { Side = BoardSide.Both; Layer = BoardLayer.UserEco; }
                                // KiCad v8+ user drawings and comments layers
                                if (name.EndsWith("_user_drawings") || name == "user_drawings") { Side = BoardSide.Both; Layer = BoardLayer.UserDrawings; }
                                if (name.EndsWith("_user_comments") || name == "user_comments") { Side = BoardSide.Both; Layer = BoardLayer.UserComments; }
                            }
                            break;
                    }
                    break;
        ...
        public static BoardFileType FindFileType(string filename)
        {
            ...
            List<string> unsupported = new List<string>() { "config", "exe", "dll", "png", "zip", "gif", "jpeg", "doc", "docx", "jpg", "bmp", "svg", "gbrjob" };
        ...
        internal static string GetStandardFileExt(BoardLayer layer, BoardSide side)
        {
            ...
            if (layer == BoardLayer.Courtyard)
            {
                if (side == BoardSide.Top) return ".top_courtyard.gbr";
                if (side == BoardSide.Bottom) return ".bottom_courtyard.gbr";
            }
            if (layer == BoardLayer.Fab)
            {
                if (side == BoardSide.Top) return ".top_fab.gbr";
                if (side == BoardSide.Bottom) return ".bottom_fab.gbr";
            }
            if (layer == BoardLayer.UserDrawings) { return ".user_drawings.gbr"; }
            if (layer == BoardLayer.UserComments) { return ".user_comments.gbr"; }
            if (layer == BoardLayer.UserEco) { return ".user_eco.gbr"; }
